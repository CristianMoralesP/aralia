(() => {
  ...
  const S =
    window.ARALIA && window.ARALIA.state
      ? window.ARALIA.state
  ...
  function resetSection(idx) {
    if (idx === 1 && card2)
      gsap.set(card2, { opacity: 0, y: 12, filter: "blur(4px)" });
    if (idx === 2 && card3)
      gsap.set(card3, { opacity: 0, x: 18, scale: 0.985, filter: "blur(6px)" });
  }
  ...
  function transitionStyle(from, to) {
    ...
    if (pair === "1->2" || pair === "2->1")
      return { duration: 1.45, ease: "expo.inOut", useFade: true, zoom: true };
    return {
      duration: 1.25,
      ease: "power2.inOut",
      useFade: false,
  ...
  function goTo(index) {
    ...
    const move = () =>
      gsap.to(pages, {
        ...
        onComplete: () => {
          S.animating = false;
          window.ARALIA?.updateCursorMode?.();
        },
      });
    ...
    if (style.useFade && fadeOverlay) {
      ...
      if (style.zoom)
        t.to(pages, { scale: 0.985, duration: 0.35, ease: "sine.inOut" }, 0);
      t.to(
        fadeOverlay,
        { opacity: 0.35, duration: 0.35, ease: "sine.inOut" },
        0,
      )
        .add(move, 0.12)
        .to(
          fadeOverlay,
          { opacity: 0, duration: 0.55, ease: "sine.out" },
          style.duration * 0.55,
        )
        .to(
          pages,
